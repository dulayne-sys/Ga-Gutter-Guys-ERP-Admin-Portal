...
export const apiVertexMeasurement = (payload: {
  ...
  request<{ status: string; text: string; result: unknown }>("vertexAIChat", {
    payload: {
...
export const apiGetLicenseStatus = () =>
  request<LicenseStatusResponse>("licenseStatus", { method: "GET" });

export const apiActivateLicense = (payload?: { plan?: string; maxUsers?: number; expiresAt?: string | null }) =>
  request<LicenseStatusResponse>("licenseActivate", { payload: payload ?? {} });

export const apiDeactivateLicense = () =>
  request<{ status: string; active: boolean }>("licenseDeactivate", { payload: {} });
...
export const apiEnrichEstimate = (payload: {
  estimateId: string;
  address: string;
  rooflineFeet?: number;
}) =>
  request<EnrichmentResponse>("enrichEstimate", { payload });

export const apiGetEstimateEnrichment = (estimateId: string) =>
  request<{ status: string; enrichment: unknown }>("getEstimateEnrichment", {
    method: "GET",
    query: { estimateId },
  });
