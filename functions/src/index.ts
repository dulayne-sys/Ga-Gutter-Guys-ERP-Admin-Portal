...
// Falls back to the request origin only if it matches the allowlist.
const ALLOWED_ORIGINS: readonly string[] = [
  // Firebase Hosting / App Hosting production domain
  "https://ga-gutter-guys-admin.web.app",
  "https://ga-gutter-guys-admin.firebaseapp.com",
  // Local development
  "http://localhost:3000",
  "http://localhost:5173",
];
...
const setCorsHeaders = (req: HttpRequest, res: HttpResponse): void => {
  ...
  // Always set Vary so caches respect the origin
  res.set("Vary", "Origin");
...
// Per-IP sliding window (resets across cold starts, which is acceptable
// for a lightweight first layer — Firestore or Redis for strict limits).
const RATE_WINDOW_MS = 60_000; // 1 minute
const RATE_MAX_REQUESTS = 60; // per window per IP

interface RateBucket { count: number; resetAt: number }
const rateBuckets = new Map<string, RateBucket>();

const isRateLimited = (req: HttpRequest): boolean => {
...
const withCors = (
  ...
  return async (req: HttpRequest, res: HttpResponse): Promise<void> => {
    ...
    // Rate-limit check (after preflight so OPTIONS aren't counted)
...
export { licenseStatus, licenseActivate, licenseDeactivate } from "./licensing/index";
export { enrichEstimate, getEstimateEnrichment } from "./ai/index";
